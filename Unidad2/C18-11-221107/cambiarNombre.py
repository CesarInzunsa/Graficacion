...
class Cuadrado():
    ...
    def trazar(self):
        ...
        glEnd()
    ###########################Estos metodos vimos hoy 07/11/2022###########################
    def trasladar(self,tx=0,ty=0):
        T = np.matrix([[1,0,tx],[0,1,ty],[0,0,1]],dtype=np.float32)
        self.transformar(T)

    def rotar(self,t=0): #grados
        t = np.radians(t)
        R = np.matrix([[np.cos(t),-np.sin(t),0],[np.sin(t),np.cos(t),0],[0,0,1]],dtype=np.float32)
        self.transformar(R)

    def escalar(self,sx=1,sy=1): 
        S = np.matrix([[sx,0,0],[0,sy,0],[0,0,1]],dtype=np.float32)
        self.transformar(S)

    def transformar(self,M): 
        i = 0
        while i < len(self.vertices):
            P = np.matrix([[self.vertices[i][0]],[self.vertices[i][1]],[1]],dtype=np.float32)
            Pp = M*P
...
def main():
    ...
    t = np.radians(45)
    r = np.matrix([[np.cos(t), -np.sin(t), 0],[np.sin(t), np.cos(t), 0],[0, 0, 1]], dtype=np.float32)
    s = np.matrix([[0.5, 0, 0],[0, 0.5, 0],[0, 0, 1]], dtype=np.float32)
